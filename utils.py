...
def select_cost(name):
    if name == 'w1':
        def cost(x, y):
            return torch.sum(torch.abs((x-y).view(x.size(0), -1)), dim=1)

    elif name == 'w2':
        def cost(x, y):
            return torch.sum(((x-y).view(x.size(0), -1))**2, dim=1)
    
    elif name == 'GW_inner':
        def cost(px, y, x):
            return -torch.sum(((px*y).view(px.size(0), -1)), dim=1)
    
    elif name == 'GW_quad':
        def cost(px, y, x):
            sq_prod = torch.sum((x.view(x.size(0), -1))**2, dim=1) * torch.sum((y.view(y.size(0), -1))**2, dim=1)
            return -torch.sum(((px*y).view(px.size(0), -1)), dim=1) -sq_prod
...
# ------------------------
class Loss:
    def __init__(self, loss_name, scheduler_name=None, num_iteration=None, alpha_min=None, alpha_max=1, warmup=0, schedule_until=None):
        self.phi = select_phi(loss_name)
        self.alpha = 1
        self.count_steps = 0
        if schedule_until is None: schedule_until=num_iteration
        self.scheduler = self.get_scheduler(scheduler_name, num_iteration, alpha_min, alpha_max, warmup, schedule_until)
    ...
    def get_scheduler(self, scheduler_name, num_iteration, alpha_min, alpha_max, warmup, schedule_until):
